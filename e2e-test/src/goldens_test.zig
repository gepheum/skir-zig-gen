...
fn evaluateTypedValue(tv: *const goldens.TypedValue, allocator: std.mem.Allocator) anyerror!goldens.TypedValue {
    return switch (tv.*) {
        .Bool,
        .Int32,
        .Int64,
...
fn typedValueTypeDescriptorJson(tv: *const goldens.TypedValue, allocator: std.mem.Allocator) ![]const u8 {
    const td = switch (tv.*) {
        .Bool => try skir_client.boolSerializer().typeDescriptor(allocator),
        .Int32 => try skir_client.int32Serializer().typeDescriptor(allocator),
        .Int64 => try skir_client.int64Serializer().typeDescriptor(allocator),
        .Hash64 => try skir_client.hash64Serializer().typeDescriptor(allocator),
        .Float32 => try skir_client.float32Serializer().typeDescriptor(allocator),
        .Float64 => try skir_client.float64Serializer().typeDescriptor(allocator),
        ...
        .EnumA => try goldens.EnumA.serializer().typeDescriptor(allocator),
        .EnumB => try goldens.EnumB.serializer().typeDescriptor(allocator),
        .KeyedArrays => try goldens.KeyedArrays.serializer().typeDescriptor(allocator),
        .RecStruct => try goldens.RecStruct.serializer().typeDescriptor(allocator),
        .RecEnum => try goldens.RecEnum.serializer().typeDescriptor(allocator),
